...
БАГ #15 ИСПРАВЛЕН: Динамический текст кнопки "Создать профиль" vs "Редактировать профиль"
"""
...
def _make_profile_keyboard(is_registered: bool = True) -> InlineKeyboardMarkup:
    """БАГ #15 ИСПРАВЛЕН: Клавиатура профиля с динамическим текстом кнопки"""
    if is_registered:
        # ЗАРЕГИСТРИРОВАННЫЙ - все кнопки
        button_text = "✏️ Редактировать профиль"
        buttons = [
            [InlineKeyboardButton(text=button_text, callback_data="profile:edit")],
            [InlineKeyboardButton(text="🗑 Удалить аккаунт", callback_data="profile:delete")],
            [InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu")]
        ]
    else:
        # НЕЗАРЕГИСТРИРОВАННЫЙ - без кнопки "Удалить аккаунт" + "Создать профиль"
