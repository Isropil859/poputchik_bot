...
# БАГ #3 ИСПРАВЛЕН: Одно окно для всех вопросов при поиске
...
async def show_all_routes(c: CallbackQuery, state: FSMContext):
    ...
    # БАГ #2 ИСПРАВЛЕН: Исключаем свои маршруты
    all_routes = [r for r in all_routes if r.get('user_id') != passenger_id]

    if not all_routes:
        # БАГ #5 ИСПРАВЛЕН ПОЛНОСТЬЮ: НИКОГДА не показываем кнопку "Создать маршрут"
        await c.message.edit_text(
            "🔭 <b>Маршруты не найдены</b>\n\n"
            "Пока нет активных маршрутов.",
...
async def start_search(c: CallbackQuery, state: FSMContext):
    """БАГ #3 ИСПРАВЛЕН: Начало ФИЛЬТРАЦИИ - запрос 'откуда'"""
    ...
    try:
        msg = await c.message.edit_text(text, reply_markup=_make_search_keyboard(show_apply=False), parse_mode="HTML")
        # БАГ #3: Сохраняем message_id для дальнейшего редактирования
        await state.update_data(bot_message_id=msg.message_id)
    except Exception:
        msg = await c.message.answer(text, reply_markup=_make_search_keyboard(show_apply=False), parse_mode="HTML")
        # БАГ #3: Сохраняем message_id для дальнейшего редактирования
        await state.update_data(bot_message_id=msg.message_id)
...
async def process_from(message: Message, state: FSMContext):
    """БАГ #3 ИСПРАВЛЕН: Обработка ответа 'откуда' - редактируем существующее сообщение"""
    data = await state.get_data()
    bot_msg_id = data.get('bot_message_id')
    
    # БАГ #3: Удаляем сообщение пользователя
    try:
    ...
    if not from_city:
        # Если пустое - редактируем существующее сообщение
        text = "❓ <b>Откуда?</b>\n\n⚠️ Пожалуйста, введите город отправления."
        try:
            await message.bot.edit_message_text(
                chat_id=message.chat.id,
                message_id=bot_msg_id,
                text=text,
                reply_markup=_make_search_keyboard(show_apply=False),
                parse_mode="HTML"
            )
        except:
            pass
    ...
    # БАГ #3: Редактируем существующее сообщение вместо создания нового
    try:
        await message.bot.edit_message_text(
            chat_id=message.chat.id,
            message_id=bot_msg_id,
            text=text,
            reply_markup=_make_search_keyboard(show_apply=True),
            parse_mode="HTML"
        )
    except:
        pass
...
async def process_to(message: Message, state: FSMContext):
    """БАГ #3 ИСПРАВЛЕН: Обработка ответа 'куда' - редактируем существующее сообщение"""
    data = await state.get_data()
    bot_msg_id = data.get('bot_message_id')
    ...
    if not to_city:
        ...
        text = (
            f"❓ <b>Куда?</b>\n\n"
            f"🔍 Откуда: {from_city}\n\n"
            f"⚠️ Пожалуйста, введите город назначения или нажмите <b>Применить</b> чтобы показать все маршруты из {from_city}:"
        )
        try:
            await message.bot.edit_message_text(
                chat_id=message.chat.id,
...
async def apply_search(c: CallbackQuery, state: FSMContext):
    ...
    # БАГ #2 ИСПРАВЛЕН: Исключаем свои маршруты
    filtered = [r for r in filtered if r.get('user_id') != passenger_id]
